...
- (id)initWithPath:(NSString*)path {
	...
	// ファイル属性取得
	attrs = [fileManager attributesOfItemAtPath:path error:NULL];       // 获取文件属性
	// 初期化
	NSMutableString* uncomp = [NSMutableString stringWithString:[path lastPathComponent]];  // 取得文件名
...
//	CFStringFold((CFMutableStringRef)uncomp, kCFCompareCaseInsensitive | kCFCompareDiacriticInsensitive | kCFCompareWidthInsensitive, NULL);
	...
	if ([work isEqualToString:NSFileTypeRegular]) {             // NSFileTypeRegular普通文件类型
	...
	if ([[attrs objectForKey:NSFileExtensionHidden] boolValue]) {       // 是否隐藏
		self.attribute |= IPMSG_FILE_EXHIDDENOPT;
	}
	if ([[attrs objectForKey:NSFileImmutable] boolValue]) {         // 只读属性
	...
	if (![self isDirectory]) {
		...
		if (osStatus != noErr) {
			...
			if (osError != noErr) {
				...
				finderFlags = info->fdFlags;    // Flags ex. kHasBundle, kIsInvisible, etc.
				// エイリアスファイルは除く------------别名文件除外
	...
	// ファイル名エスケープ（":"→"::"）-----------文件名（":"→"::"）转义
	range = [self.name rangeOfString:@":"];     // Nsstring中搜索方法rangeOfString
	if (range.location != NSNotFound) {
		NSMutableString*	escaped	= [NSMutableString stringWithCapacity:[self.name length] + 10];
		NSArray*			array	= [self.name componentsSeparatedByString:@":"];    // 根据:分割成数组
		unsigned			i;
