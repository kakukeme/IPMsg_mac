...
- (id)initWithSendMessage:(NSString*)msg recvMessage:(RecvMessage*)recv {
	...
	if (msg) {
		if ([msg length] > 0) {
			// 引用文字列行末の改行がなければ追加----msg不是换行符结束，添加“\n”
...
- (void)sheetDidEnd:(NSWindow*)sheet returnCode:(int)code contextInfo:(void*)info {
	if (info == sendButton) {
		[sheet orderOut:self];
		if (code == NSOKButton) {
			// 不在モードを解除してメッセージを送信--------发送消息来取消自动模式
...
- (IBAction)sendPressed:(id)sender {
	...
	if (config.inAbsence) {
		// 不在モードを解除して送信するか確認-------请务必将它发送到释放无人参与模式
	...
	// 受信ウィンドウ消去（初期設定かつ返信の場合）----------------（在回答和默认的情况下）接收窗口擦除
...
// SplitViewのリサイズ制限-------------调整SPLITVIEW极限
- (float)splitView				:(NSSplitView*)sender
		  constrainMinCoordinate:(float)proposedMin
					 ofSubviewAt:(int)offset {
	if (offset == 0) {
		// 上側ペインの最小サイズを制限--------它限制了上部窗格的最小尺寸
		return 90;
	}
	return proposedMin;
}
...
- (float)splitView				:(NSSplitView*)sender
		  constrainMaxCoordinate:(float)proposedMax
					 ofSubviewAt:(int)offset {
	if (offset == 0) {
		// 上側ペインの最大サイズを制限--------限制上部窗格中的最大尺寸
		return [sender frame].size.height - [sender dividerThickness] - 2;
	}
	return proposedMax;
}

// SplitViewのリサイズ処理-----------SPLITVIEW的重新调整过程
- (void)splitView:(NSSplitView*)sender resizeSubviewsWithOldSize:(NSSize)oldSize
{
	...
	if (frame1.size.height > newSize.height - divider) {
		// ヘッダ部の高さは変更しないがSplitViewの大きさ内には納める---不要更改标题部分的高度，但在适合的大小SPLITVIEW
...
/*----------------------------------------------------------------------------*
 * NSTableDataSourceメソッド------------NSTableDataSource方法
 *----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*
 * NSTableViewDelegateメソッド-------NSTableViewDelegate方法
 *----------------------------------------------------------------------------*/
