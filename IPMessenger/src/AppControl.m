...
/*----------------------------------------------------------------------------*
 * メッセージ送受信／ウィンドウ関連------------可以接发消息/ windows相关
 *----------------------------------------------------------------------------*/

// 新規メッセージウィンドウ表示処理----------新信息处理显示windows
...
- (void)applicationDidFinishLaunching:(NSNotification*)aNotification
{
	...
	// ログファイルのUTF-8チェック
	TRC(@"Start log check");
	Config* config = [Config sharedConfig];         // 初始化Config
	...
	// ENTRYパケットのブロードキャスト
	TRC(@"Broadcast entry");
	[[MessageCenter sharedCenter] broadcastEntry];      // 登录，发送用户上线ENTRY广播
...
- (void)applicationWillTerminate:(NSNotification*)aNotification {
	// EXITパケットのブロードキャスト
	[[MessageCenter sharedCenter] broadcastExit];       // 用户退出
	// 添付ファイルサーバの終了
