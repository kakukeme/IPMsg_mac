...
// 次のメッセージIDを返す------我返回的下一个消息ID-----流水号
+ (long)nextMessageID {
	static long messageID = 0;
	return ++messageID;
}

// ネットワークに接続しているかを返す------是否连接到网络
...
- (id)init
{
	...
	// DynamicStore更新通知設定----DynamicStore更新通知设定
	...
	// DynamicStoreからの情報取得--------DynamicStore信息采集
	[self updateHostName];
...
- (NSInteger)sendTo:(struct sockaddr_in*)toAddr messageID:(NSInteger)mid command:(UInt32)cmd data:(NSData*)data
{
	...
	[sockLock unlock];	// ロック解除-------所解除

	return mid;
}

// 带有选项的消息发送，用户信息UserInfo
...
{
	// ブロードキャスト（ローカル）アドレスへ送信--------发送到（本地）广播地址
...
// BR_ABSENCEのブロードキャスト---------BR_ABSENCE广播
...
- (void)sendMessage:(SendMessage*)msg to:(NSArray*)toUsers
{
	...
	// コマンドの決定---------命令的测定
	if ([toUsers count] > 1) {
		command |= IPMSG_MULTICASTOPT;          // 发送给多个人
	}
	if (msg.sealed) {
		command |= IPMSG_SECRETOPT;             // 安全
		if (msg.locked) {
			command |= IPMSG_PASSWORDOPT;       // 加锁，需要密码
		}
	}

	// 添付ファイルメッセージ編集----------附加文件信息编辑----附件信息
	...
	if (retryInfo) {
		if (retryInfo.retryCount >= RETRY_MAX) {
			...
			if (ret == NSAlertAlternateReturn) {
				// 再送キャンセル----------转播取消
				// 応答待ちメッセージ一覧からメッセージのエントリを削除-----从列表信息等应答删除下一个信息
